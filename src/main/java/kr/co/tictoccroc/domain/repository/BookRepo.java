...
public interface BookRepo extends JpaRepository<Book, Long> {
  ...
  @Query("SELECT b FROM Book b JOIN FETCH b.storeLesson sl JOIN FETCH sl.lesson JOIN FETCH sl.store WHERE b.delAt IS NULL AND sl.delAt IS NULL AND b.bookStatus <> 'CANCEL' AND sl.store.id = :storeId")
  List<Book> findByStore(@Param("storeId") long storeId);


  @Query("SELECT b FROM Book b JOIN FETCH b.storeLesson sl JOIN FETCH sl.lesson JOIN FETCH sl.store WHERE b.delAt IS NULL AND sl.delAt IS NULL AND b.bookStatus <> 'CANCEL' AND sl.lesson.id = :lessonId")
